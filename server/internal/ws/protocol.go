...
// panel (backup now / restore / policy push), and live progress/log
// streaming back to the server. Bulk data (chunk upload/download,
// manifests) travels over plain HTTP instead - see internal/api - so this
// channel stays small and responsive even while a multi-gigabyte backup is
// in flight on the same connection's HTTP side.
package ws
...
type Envelope struct {
	...
	Phase         string  `json:"phase,omitempty"` // scanning/uploading/finalizing/restoring
	...
	// agent -> server: backup_finished / restore_finished
...
const (
	...
	TypeRestoreStarted  = "restore_started"  // agent -> server
	TypeRestoreFinished = "restore_finished" // agent -> server
)
